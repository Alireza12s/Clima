...
class WeatherViewController: UIViewController, CLLocationManagerDelegate, ChangeCityDelegate{
    ...
    let APP_ID = "bd140c2afca03163245d18476450717a"

    //TODO: Declare instance variables here
    let locationManager = CLLocationManager()
    let wetherDataModel = WeatherDataModel()
    ...
    func changeCelsiusFahrenheit(){
        
        
        if self.segmentedControl.selectedSegmentIndex == 0{
            temperatureLabel.text = String(wetherDataModel.temperature) + "°"
            
        } else {
             let tmp = wetherDataModel.temperature
            let t = Int(Double(tmp) + 273.15)
                temperatureLabel.text  = String(t)
